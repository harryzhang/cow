package com.redpack.common.util;
...
public class StringUtil {
	...
	public static String FilteSqlInAndScript(String input) {
		...
		if (!StringUtils.isNumeric(input)) {
			return input.replaceAll("'", "’")
					.replaceAll("update", "ｕｐｄａｔｅ")
					.replaceAll("select", "s e l e c t")
					.replaceAll("SELECT", "s e l e c t")
					.replaceAll("UPDATE", "ｕｐｄａｔｅ")
					.replaceAll("drop", "ｄｒｏｐ")
					.replaceAll("DROP", "ｄｒｏｐ")
					.replaceAll("delete", "ｄｅｌｅｔｅ")
					.replaceAll("DELETE", "ｄｅｌｅｔｅ")
					.replaceAll("exec", "ｅｘｅｃ")
					.replaceAll("EXEC", "ｅｘｅｃ")
					.replaceAll("create", "ｃｒｅａｔｅ")
					.replaceAll("CREATE", "ｃｒｅａｔｅ")
					.replaceAll("execute", "ｅｘｅｃｕｔｅ")
					.replaceAll("EXECUTE", "ｅｘｅｃｕｔｅ")
					.replaceAll("where", "ｗｈｅｒｅ")
					.replaceAll("WHERE", "ｗｈｅｒｅ")
					.replaceAll("truncate", "ｔｒｕｎｃａｔｅ")
					.replaceAll("TRUNCATE", "ｔｒｕｎｃａｔｅ")
	...
	public static String decodeNetUnicode(String str) {
		...
		for (; m.find(); m.appendReplacement(sb, Matcher.quoteReplacement(s))) {
			...
			s = charValue <= 0 ? "" : (new StringBuilder(
					String.valueOf((char) charValue))).toString();
		}
